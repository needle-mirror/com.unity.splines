...
namespace UnityEngine.Splines
{
	...
	public static class CurveUtility
    {
        ...
        internal static void CalculateCurveLengths(BezierCurve curve, List<ISpline.DistanceToTime> lookupTable)
        {
            lookupTable.Clear();
            var resolution = lookupTable.Capacity;

            float magnitude = 0f;
            float3 prev = EvaluatePosition(curve, 0f);
            lookupTable.Add(new ISpline.DistanceToTime() { distance = 0f , time = 0f});
            ...
            for (int i = 1; i < resolution; i++)
            {
                ...
                lookupTable.Add(new ISpline.DistanceToTime() { distance = magnitude , time = t});
