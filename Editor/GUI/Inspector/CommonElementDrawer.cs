namespace UnityEditor.Splines
{
    ...
    {
        readonly TangentModeDropdown<ISplineElement> m_Mode;
        readonly BezierTangentModeDropdown<ISplineElement> m_BezierMode;

        public CommonElementDrawer()
        {
            Add(m_Mode = new TangentModeDropdown<ISplineElement>());
            m_Mode.changed += () => { m_BezierMode.Update(targets); };

            Add(m_BezierMode = new BezierTangentModeDropdown<ISplineElement>());
